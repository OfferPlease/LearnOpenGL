...
void initScene(int argc, char* argv[])
{
	...
	size_t width = heightmapImage->getHeight();
	size_t height = heightmapImage->getHeight();
	numVertices = width * height; // This must be a global variable, so that we know how many vertices to render in glDrawArrays.

	// Vertex positions.
	// first try for point modes
	float* positions = (float*)malloc(numVertices * 3 * sizeof(float)); // 3 floats per vertex, i.e., x,y,z
	// positions are each pixel position
	for (size_t z = 0; z < height; z++)
	{
		for (size_t x = 0; x < 3 * width; x += 3)
		{
			positions[z * width * 3 + x] = (float)x / (float)(width - 1); // x value for coordinate i,j
			positions[z * width * 3 + x + 1] = (float)heightmapImage->getPixel(x/3, z, 0) / 255.0f; // y value for the coordinate i, j
			positions[z * width * 3 + x + 2] = (float)z / (float)(height - 1);
		}
	}
	...
	for (size_t z = 0; z < height; z++)
	{
		...
		{
			colors[z*width*4+x] = 1.0; 
			colors[z * width * 4 + x+1] = 1.0;  
			colors[z * width * 4 + x + 2] = 1.0;
			colors[z * width * 4 + x + 3] = 1.0; // (r,g,b,a) channels of the vertex
		}
	}
